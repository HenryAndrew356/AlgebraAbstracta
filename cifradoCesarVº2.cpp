//  Actividad:  Cifrado Cesar    - Cifrado/Descifrado/Criptoanalisis
...
string libreriaPalabrasEspañol[250]={"Hola","Versace"};

void Cifrado(string &mensajeAencriptar);
void Descifrado(string &mensajeEncriptado);

void criptoanalisis(string &mensaje);
...
void criptoanalisis(string &mensaje)
{
    string palabras123[100];
    string mensajePivote=mensaje;
    char espacio=' ';
    int posiciones[40];
    int b{0};
    for(unsigned int a{0};a<(mensaje.size());a++)
    {
        if((mensaje[a])==espacio)
        {
            posiciones[b]=a;
            b++;
        }
